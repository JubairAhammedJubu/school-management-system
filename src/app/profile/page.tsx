...
export default function ProfilePage() {
  ...
  const [bio, setBio] = useState("Dedicated educator passionate about interactive learning and STEM education.");
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 px-4 py-24 flex items-center justify-center">
        ...
        >
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Please log in to your EduNexus account to access your user profile and settings.
          </p>
          <Link
            href="/login"
  ...
  const userRole = ((session?.user as { role?: string } | undefined)?.role || "Student").toUpperCase();
  ...
  const userCreatedAt = session?.user?.createdAt
    ...
    : "Jan 2026";
  const userUpdatedAt = (session?.user as { updatedAt?: string | Date } | undefined)?.updatedAt
    ? new Date((session?.user as { updatedAt?: string | Date }).updatedAt!).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
                <div className="relative h-28 w-28 sm:h-32 sm:w-32 rounded-3xl border-4 border-white dark:border-slate-900 bg-gradient-to-tr from-indigo-600 to-purple-600 text-white shadow-2xl flex items-center justify-center text-4xl font-extrabold shrink-0 overflow-hidden group">
                  ...
                  <button
                    ...
                    className="absolute inset-0 bg-slate-950/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-xs font-semibold text-white cursor-pointer"
                  >
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xl backdrop-blur-xl">
                    ...
                    <div className="space-y-3 text-xs">
                      ...
                      <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <span className={`inline-flex items-center gap-1 font-bold ${isEmailVerified ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}>
                          <CheckCircle2 className="h-3.5 w-3.5" />
                      ...
                      {/* Last Profile Update */}
                      <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50">
                        <span className="text-slate-600 dark:text-slate-400">Last Profile Update</span>
                        <span className="font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1">
                  ...
                  <div className="rounded-3xl border border-indigo-200/80 dark:border-indigo-900/60 bg-gradient-to-br from-indigo-50/80 to-purple-50/60 dark:from-indigo-950/40 dark:to-slate-900 p-6 shadow-lg">
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                      Your EduNexus profile ID links your digital attendance, course materials, grade submissions, and school notices automatically across the system.
                    </p>
                  </div>
                </div>
              </motion.div>
