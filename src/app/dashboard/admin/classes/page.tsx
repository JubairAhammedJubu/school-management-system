      ...
      >
        <div>
          ...
          <p className="mt-1.5 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Class 9–10 show groups (Science, Business Studies, Humanities).
            Click a section for roster & teachers.
          </p>
        </div>
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="h-40 rounded-2xl bg-slate-100 dark:bg-slate-900 animate-pulse"
            />
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-950 px-6 py-14 text-center shadow-sm">
          ...
          <h3 className="mt-3 text-sm font-bold">No classes found</h3>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
