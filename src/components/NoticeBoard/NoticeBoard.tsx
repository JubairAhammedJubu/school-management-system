...
import { Bell, CalendarDays, Sparkles, Search, Tag, Pin, Plus, X, CheckCircle2, User, Loader2, GraduationCap, ChevronDown, Check } from "lucide-react";


import { motion, AnimatePresence } from "framer-motion";
import { toast } from "react-toastify";
import { useSession } from "@/lib/auth-client";
import { getNoticesAction, createNoticeAction, NoticeItem } from "@/lib/actions/teacher.notice";
...
export default function NoticeBoard({
  ...
  const handleCreateNotice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentUserName = (session?.user as { name?: string } | undefined)?.name || session?.user?.email || "Teacher";
      ...
      if (res.success && res.notice) {
        ...
        setNotices((prev) => [formattedNotice, ...prev.filter((n) => n.id !== formattedNotice.id)]);
              ...
              <form onSubmit={handleCreateNotice} className="space-y-3.5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    {/* Custom Select Popover Dropdown */}
                    <AnimatePresence>
